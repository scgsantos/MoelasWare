...
function CreateTest() {
    ...
    function quizHasMatchingTag(quiz, tag) {
        for (let i = 0; i < quiz.tags.length; i++) {
            if (
                quiz.tags[i].text.toUpperCase().indexOf(tag.toUpperCase()) === 0
            ) {
                return true;
            }
        }
    ...
    function handleSearchChange(e) {
        ...
        if (e.target.value !== "") {
            ...
            setPageTotal(
                Math.ceil(quizzes_matching_search.length / quizzes_per_page)
            );
        } else {
    ...
    function handleQuizSelectionChange(quiz) {
        if (quizHasBeenSelected(quiz)) {
            var q = quizzes.filter(function (q) {
                return q !== quiz.id;
            });
    ...
    function getPaginationArray() {
        if (pagination) {
            return searched_quizzes.slice(
                (page_num - 1) * quizzes_per_page,
                page_num * quizzes_per_page
            );
        } else {
    ...
    function handleQuizPreview(quiz) {
        API.getQuizAnswers(quiz.id).then((data) => {
            setPopUp(
                <div className="SelectQuizzes-windowList">
                    <h1 className="SelectQuizzes-windowWrapper">
                        {quiz.question}
                    </h1>
        ...
        if (quizHasBeenSelected(quiz)) {
            return (
                <li
                    ...
                    onClick={() => handleQuizSelectionChange(quiz)}
                >
                    <p
                        style={{ color: "green" }}
    ...
    function renderQuizList(id) {
        ...
        for (let i = 0; i < all_quizzes.length; i++) {
            if (all_quizzes[i].id == id)
                return (
                    <li className="NumberOnly-quiz">{all_quizzes[i].name}</li>
                );
        }
    }
        ...
        return (
            <div className="NumberOnly-firstPage" onClick={() => handleClick()}>
                ...
                <section id="createTest">
                    ...
                    &nbsp;&nbsp; &nbsp;&nbsp;
                    <button
                        className="createTestButtonn"
                        onClick={handleCreateButtonChange}
                    >
                        Next
                    </button>
                </section>
