...
class QuizList extends React.Component {
  ...
  prevQuiz(id) {
    if (this.state.quiz_id - 1 < 0) {
      ...
      this.getQuizAnswers(
        this.state.quizzes[this.state.quizzes.length - 1]?.id
      );
    } else {
      this.setState({ quiz_id: this.state.quiz_id - 1 });
      this.getQuizAnswers(this.state.quizzes[this.state.quiz_id - 1]?.id);
    ...
    if (answer?.correct) {
      return (
        <li className="quiz-correct">
          {" "}
          Answer {this.state.i++}: &ensp;{answer?.text}{" "}
        </li>
      );
    } else {
      return (
      ...
      <div>
        ...
        <div className="preview-container">
          ...
          <h1 className="preview-subTitle">
            Quizz #{this.state.quiz_id + 1} &ensp; {quiz?.question}
          </h1>
        ...
        <div className="preview-container">
          <button className="preview-buttonPublish">Go Back</button>
          &ensp;
          <button
            className="preview-buttonPublish"
